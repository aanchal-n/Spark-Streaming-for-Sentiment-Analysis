...
def process_rdd(record, spark, models): 
    ...
    if not record.isEmpty():
        ...
        tweets_shuffled = [tweets[i] for i in indices]

        split_tweets = list(map(preprocessing.preprocess_tweet, tweets_shuffled)) # Preprocess tweets and re-sentence
        tweet_sentences = list([' '.join(split_tweet) for split_tweet in split_tweets])
        

        batch_results = []
        for model in models:
            model.fit(tweet_sentences, labels_shuffled) # Fit model using a HashingVectorizer, and split dataset into training and validation
...
if __name__ == "__main__":
    ...
    classifiers_and_parameters = {
                                SGDClassifier: {"alpha":0.0001, "eta0":0.001, "power_t": 0.5, "loss": "modified_huber", "penalty": "elasticnet"},
                                MultinomialNB: {"alpha":0.85},             
                                PassiveAggressiveClassifier: {"C":0.1, 'loss': 'squared_hinge'}
                                 }
    
    models = [model.Model(classifier, classifier.__name__, classifiers_and_parameters[classifier]) for classifier in classifiers_and_parameters]
